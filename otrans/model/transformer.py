...
class Transformer(nn.Module):
    ...
    def forward(self, inputs, inputs_length, targets, targets_length):
        ...
        #Args:
        #    inputs: padding data of batch frames, [batch, max_seq_len, feat_dim]
        #    inputs_length: length of padding batch data, [batch]
        #    targets: padding words of batch, [batch, max_word_len]
        #    targets_length: length of padding batch words, [batch]

        # 1. forward encoder
        enc_states, enc_mask = self.encoder(inputs, inputs_length)
        # enc_states: with shape [batch, max_seq_len, emd_dim], 
        #             used input for K and V of decoder src-attention,
        #             so its shape is same as embedding of decoder
        # enc_mask: with shape [batch, 1, max_seq_len]
        #           [1, max_seq_len] used to mask soft_max value
